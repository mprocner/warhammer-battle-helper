...
6. Otwórz `/privacy` bez zalogowania → strona się renderuje, a pod tekstem polityki widoczny jest przełącznik zgody (bez konieczności logowania).
7. **Host `www.`** (fix wave, I4): dodaj wpis do `/etc/hosts` mapujący `www.playrpg.local` na `127.0.0.1` albo użyj `--host` wspieranego przez `npm start`, tak żeby `window.location.hostname` zaczynało się od `www.`. Zaakceptuj zgodę, poczekaj aż powstanie cookie `_ga`, potem wyłącz przełącznik w Ustawieniach → Prywatność. Sprawdź w DevTools → Application → Cookies, że `_ga`/`_ga_<ID>` zniknęły — na `localhost` ten test nic by nie wykrył, bo tam działa nawet samo kasowanie host-only.
8. **Wycofanie w drugiej karcie** (fix wave, I5): zaakceptuj zgodę, otwórz stronę w drugiej karcie tej samej przeglądarki (nie w trybie prywatnym — `storage` event nie leci między oknami prywatnymi/normalnymi). W jednej karcie wejdź w Ustawienia → Prywatność i wyłącz przełącznik. W **drugiej** karcie, bez odświeżania, sprawdź że kolejna nawigacja/akcja nie dopisuje niczego do `window.dataLayer` — zgoda poszła za `storage` eventem.
9. **Pierwszy pageview po akceptacji** (fix wave, I3): wyczyść `localStorage`, odśwież `/login` (baner widoczny, `window.dataLayer` puste — brak page_view, bo GA jeszcze wyłączone). Kliknij „Akceptuję" **i nie nawiguj nigdzie dalej**. Sprawdź `window.dataLayer` — musi zawierać `['event','page_view',{page_path:'/login', page_location:...}]`, mimo że ścieżka się nie zmieniła.
10. **Token nie ucieka przez `dl`** (fix wave, C1): zakładka Network, filtr `google-analytics.com/g/collect` albo `collect?`. Wejdź na `/verify-email?token=<cokolwiek>` po zaakceptowaniu zgody. W parametrach żądania `collect` sprawdź parametr `dl` (document location) — musi być samym originem + `/verify-email`, bez `?token=` i bez wartości tokenu. To samo dotyczy `/reset-password?token=...`.
11. **`form_destination` nie ucieka na `form_start`** (fix wave 3, C1 na `form_destination`): jedyny sposób, żeby faktycznie dowieść, że ta poprawka działa — żaden test w repo tego nie zastąpi.

    **Kolejność jest istotna: „Form interactions” musi być na tym etapie WŁĄCZONE.** To ustawienie generuje zdarzenie `form_start`, czyli dokładnie to, co tu weryfikujemy. Wyłączenie go przed sprawdzeniem (krok 12) sprawia, że nie ma czego szukać — i łatwo wtedy uznać brak żądania za dowód poprawności, choć niczego nie dowodzi.

    Przy udzielonej zgodzie wejdź na `/reset-password?token=CANARY` i kliknij w pole nowego hasła (pierwsza interakcja z formularzem). W DevTools → Network filtruj po `collect` — nie po `collect?v=2`, bo to gubi część wariantów.

    **GA4 paczkuje zdarzenia**: pojedyncze zdarzenie niesie parametry w query stringu, ale kilka naraz idzie jako `POST` na `/g/collect` z treścią w **Request Payload**. Sprawdzaj obie zakładki, inaczej przegapisz zdarzenie. `form_start` odpala się raz na formularz na sesję — po wcześniejszym kliknięciu w pole trzeba odświeżyć stronę, żeby wystrzeliło ponownie.

    Znajdź wpis z `en=form_start` i sprawdź, że `dl`, `dr` oraz `ep.form_destination` nie zawierają ani `?`, ani `CANARY`.

    Wygodniejsza alternatywa: rozszerzenie *Google Analytics Debugger* + **Admin → DebugView** w GA. Kliknięcie zdarzenia `form_start` na osi rozwija jego parametry z wartościami. Czytelniejsze niż surowe żądania; Network zostaje jako rozstrzygający, gdyby DebugView czegoś nie pokazał.

    To jest dokładnie ten test, który złapałby wszystkie trzy wektory wycieku z tego feature'u (`page_location`, `page_referrer`, `form_destination`) — żaden test jednostkowy w repo nie sprawdza realnego żądania `collect`, więc to jedyna rzeczywista weryfikacja.
12. **Dopiero PO kroku 11 — enhanced measurement w konsoli GA4**: mając potwierdzenie z kroku 11, rozważ wyłączenie „Form interactions” w ustawieniach enhanced measurement strumienia danych GA4 jako dodatkowe zabezpieczenie (belt-and-braces) — chroni przed formularzem dodanym kiedyś na route z sekretem w URL-u. To ustawienie konsoli, niewidoczne z poziomu repo: uzupełnia poprawkę w kodzie, nigdy jej nie zastępuje. Nie wyłączaj go wcześniej — krok 11 przestaje wtedy cokolwiek weryfikować.
