...
11. **`form_destination` nie ucieka na `form_start`** (fix wave 3, C1 na `form_destination`): jedyny sposób, żeby faktycznie dowieść, że ta poprawka działa — żaden test w repo tego nie zastąpi. Przy udzielonej zgodzie wejdź na `/reset-password?token=CANARY`, kliknij w pole nowego hasła (pierwsza interakcja z formularzem), w DevTools → Network odfiltruj `collect?v=2`, znajdź żądanie z `en=form_start` i sprawdź, że `dl`, `dr` oraz `ep.form_destination` nie zawierają ani `?`, ani `CANARY`. To jest dokładnie ten test, który złapałby wszystkie trzy wektory wycieku z tego feature'u (page_location, page_referrer, form_destination) — żaden test jednostkowy w repo nie sprawdza realnego żądania `collect`, więc to jedyna rzeczywista weryfikacja.
12. **Uwaga przed wdrożeniem — enhanced measurement w konsoli GA4**: rozważ wyłączenie „Form interactions” w ustawieniach enhanced measurement strumienia danych GA4 jako dodatkowe zabezpieczenie (belt-and-braces). To ustawienie konsoli, niewidoczne z poziomu repo — uzupełnia poprawkę w kodzie, nigdy jej nie zastępuje.
