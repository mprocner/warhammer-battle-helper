...
/**
 * Builds the correct API payload for a character save.
 * For Warhammer: re-wraps root-level stats fields back into `stats` before sending.
 * For other systems: returns the character as-is.
 */
export function buildPayload(charToSave) {
    if (!charToSave.gameSystem || charToSave.gameSystem === 'warhammer4e') {
        const topLevel = {};
        const statsFields = {};
