...
func (r *GameRepository) UpdateScene(gameID string, sceneID primitive.ObjectID, req models.UpdateSceneRequest) error {
	...
	if req.SceneMusicLoop != nil {
		setFields["scenes.$.sceneMusicLoop"] = *req.SceneMusicLoop
	}
...
// SetImageUrl sets (or clears, when empty) the lobby image URL of a game.
func (r *GameRepository) SetImageUrl(gameID string, imageUrl string) error {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
...
// SetCustomTemplate replaces the embedded customSystemTemplate in a game document.
func (r *GameRepository) SetCustomTemplate(gameID string, tmpl *models.SystemTemplate) error {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	...
	_, err = r.Collection.UpdateOne(ctx,
		bson.M{"_id": objectID},
		bson.M{"$set": bson.M{"customSystemTemplate": tmpl, "updatedAt": time.Now()}},
	)
	return err
}
